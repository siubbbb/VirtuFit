...
import { Camera, Loader2, PersonStanding, Check, RefreshCw, AlertTriangle, ArrowRight, Video } from 'lucide-react';
...
type CaptureStep = 'initial' | 'camera-ready' | 'front-captured' | 'processing' | 'error';
...
export function CameraCapture() {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
      try {
        ...
        toast({
          variant: 'destructive',
          title: 'Camera Error',
          description: 'Could not access the camera. Please check permissions in your browser settings and try again.',
        });
      }
    } else {
        setStep('error');
  ...
  const getGuideText = () => {
    switch (step) {
      case 'initial': return 'Press the button below to start your camera.';
      case 'camera-ready': return 'Face forward and align with the guide.';
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="w-full max-w-md">
        {step === 'initial' ? (
           <Button onClick={setupCamera} className="w-full" size="lg">
        ...
        {step === 'camera-ready' ? (
           <Button onClick={handleCapture} disabled={isProcessing} className="w-full" size="lg">
              <Camera className="mr-2" />
              Capture Photo
            </Button>
        ) : null}
