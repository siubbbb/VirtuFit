...
export function QrCodeFlow() {
  ...
  // Get the current URL origin on the client side
  useEffect(() => {
  ...
  const qrUrl = useMemo(() => {
    ...
    // The URL the QR code will point to. It includes the user's UID to associate
    // the mobile session with the desktop session.
